...
//      com.amazonaws.auth.DefaultAWSCredentialsProviderChain.class;
    ...
    public SqsClient(Map<String, ?> configs) {
        log.debug(".ctor:configs={}", configs);
...
//    log.info("AmazonSQS using profile={}, region={}", profile, region);
    ...
    /**
     * Batch delete a message from the SQS queue.
     *
     * @param url      SQS queue url.
     * @param receipts Message receipt handles of messages to delete.
     * @return Receipt handles that could not be deleted
     */
    public List<PendingReceipt> batchDelete(final String url, final List<PendingReceipt> receipts) {
        Guard.verifyValidUrl(url);
        Guard.verifyNotNull(receipts, "receiptHandles");
        Guard.verifyInRange(receipts.size(), 1, 10, "receiptHandles");

        final List<DeleteMessageBatchRequestEntry> entries = receipts.stream()
                .map(receiptHandle -> new DeleteMessageBatchRequestEntry(receiptHandle.getHandle(), receiptHandle.getHandle()))
                .collect(Collectors.toList());
        final DeleteMessageBatchResult result = client.deleteMessageBatch(url, entries);

        Set<String> failedHandles = result.getFailed().stream().map(BatchResultErrorEntry::getId).collect(Collectors.toSet());
        final List<PendingReceipt> failedReceiptHandles =
                result.getSdkHttpMetadata().getHttpStatusCode() != 200
                        ? receipts
                        : receipts.stream().filter(receipt -> failedHandles.contains(receipt.getHandle())).collect(Collectors.toList());

        if (failedReceiptHandles.isEmpty()) log.debug(".delete:receipt-handles={}", receipts);
        else log.warn(".delete:receipt-handles={}, failed={}", receipts, failedReceiptHandles.size());

        return failedReceiptHandles;
    }
